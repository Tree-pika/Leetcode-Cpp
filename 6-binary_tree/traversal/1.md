...
* solution
  ...
  * 线索二叉树Threaded Binary Tree：Morris遍历
    ...
    * Morris的核心思想：利用叶子节点（Left Subtree's Rightmost Node）大量的空闲指针 (right == NULL)，指向当前节点的后继（Back to Root），从而省去栈的空间。
    * time complexity：$O(N)$
    ...
    * 理解：Morris 遍历本质上是一个“会回到原点”的过程。
      ...
      * inorder
        * 如果 root 有左孩子，第一次见到它时不打印，先去遍历左边。等左边都跑完了，顺着线索第二次回到 root 时，才打印它。
